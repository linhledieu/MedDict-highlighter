...
class MedDictHighlighter extends HTMLElement {
  ...
  highlightSelection() {
    ...
    for (let i = 0; i < userSelection.rangeCount; i++) {
      ...
      const word = capitalize(range.toString());
      const search_word = lowerCase(range.toString());

      // send request to the server
      // http://localhost/search?word=search_word&lang=en
      // http://localhost/search?word=search_word&lang=vn 
      // compare the result. 
      
      // send request to the server, write js :) send no js code pls
      let imgID = 0;
      let language = 'en';
      ...
      fetch(englishUrl)
        ...
        .then(data => {
          ...
          if (data.length == 0) {
            ...
            console.log(vietnameseUrl);
            return fetch(vietnameseUrl);
          }
          data = data[0];
          console.log(data);
          const meanings = [data["vn"]];
          imgID = data["id"];
